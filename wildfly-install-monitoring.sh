...
WILDFLY_USER="monitoring"
WILDFLY_SERVICE="monitoring"
...
if [ "x$1" = "xdomain" ]; then
    ...
    $WILDFLY_HOME/bin/standalone.sh -Djboss.node.name=node0
    #>> /var/log/$WILDFLY_SERVICE/server-`date +%Y-%m-%d`.log
fi
EOF
    # $WILDFLY_HOME is not visible here
    sed -i -e 's,WILDFLY_HOME=.*,WILDFLY_HOME='$WILDFLY_DIR',g' $WILDFLY_DIR/bin/launch.sh
    sed -i -e 's,$WILDFLY_HOME/bin/standalone.sh.*,$WILDFLY_HOME/bin/standalone.sh -c $2 -b $3 -Djboss.node.name=node0,g' $WILDFLY_DIR/bin/launch.sh
...
start-stop-daemon --start --background --chuid $WILDFLY_USER --exec $WILDFLY_DIR/bin/standalone.sh -Djboss.node.name=node0
exit $?
;;
stop)
...
ECLIPSE_LINK_DOWNLOAD_ADDRESS=http://central.maven.org/maven2/org/eclipse/persistence/eclipselink/2.5.0/eclipselink-2.5.0.jar
ECLIPSE_DIR=$WILDFLY_DIR/modules/system/layers/base/org/eclipse/persistence/main
wget -O $ECLIPSE_DIR/eclipselink.jar  $ECLIPSE_LINK_DOWNLOAD_ADDRESS
